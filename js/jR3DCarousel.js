...
(function (factory) {
	...
	$.fn.jR3DCarousel = function(options){
		...
		(function setup(){
			...
			function _createControls(){
				...
				_previousButton.add(_nextButton)
							   .css({position: 'absolute', top:'50%', zIndex:1, transform: 'translateY(-50%)', fontSize: '4em', color: 'rgba(0, 0, 0, 0.7)', cursor:'pointer', userSelect: 'none'})
							   .appendTo(_container)
							   .hide();
				...
				_previousButton.hover(function(){
					...
					_previousButton.css({color: 'rgba(0, 0, 0, 0.7)'});
				});
				_nextButton.hover(function(){
					_nextButton.css({color: 'rgba(201, 201, 201, 0.97)'});
				},function(){
					_nextButton.css({color: 'rgba(0, 0, 0, 0.7) '});
				});
		...
		function _slideCarouseld(){
			_container.find('.nav').css({  backgroundColor: 'rgba(0, 0, 0, 0.77)' })
